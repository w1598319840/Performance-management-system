		...
		if (e.getSource() == submit) {
			...
			if (ch == "学生") {
				if ((new CheckInfo().isMember("student", idt.getText(), "000") == 2)) {
					...
					File leaveFile = new File(
							System.getProperty("user.dir") + "/data/leave_records/" + idt.getText() + ".txt");
					leaveFile.delete();
				...
				if ((new CheckInfo().isMember("administrator", idt.getText(), "000") == 2)) {
					...
					JOptionPane.showMessageDialog(null, "删除管理员成功！", "提示", JOptionPane.INFORMATION_MESSAGE);
				} else {
					JOptionPane.showMessageDialog(null, "此管理员不存在！", "提示", JOptionPane.INFORMATION_MESSAGE);
				}
