...
public class EditInfo extends JFrame implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == submit) {
			if ((inst.getText().equals("")) || (birtht.getText().equals("")) || (namet.getText().equals(""))
				...
				if (!(pass1t.getText().equals(pass2t.getText()))) {
					...
					if (flag == 0) { // 学生修改信息
						...
						try {
							...
							while ((s = br.readLine()) != null) {
								...
								if (result[0].equals(id)) {
									...
									result[1] = result[1].replace(result[1], pass2t.getText());
									result[2] = result[2].replace(result[2], namet.getText());
									result[3] = result[3].replace(result[3], m);
									result[4] = result[4].replace(result[4], birtht.getText());
									result[5] = result[5].replace(result[5], instt.getText());
									result[6] = result[6].replace(result[6], majort.getText());
						...
						ArrayList<String> modifiedContent = new ArrayList<String>();
						String file = System.getProperty("user.dir") + "/data/administrator.txt";
